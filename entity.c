...
// Student names: Elena Long
// Student computing IDs: ml6vq
//
//
...
void A_init(void) {
    ...
    A.nextseq = 1;
    A.window_size = 8;
    A.estimated_rtt = 15.0;
    A.buffer_next = 1; //also keep track of seq number
}
...
void B_input(struct pkt packet) {
    if (packet.checksum != get_checksum(&packet) || packet.seqnum != B.expect_seq) {
        //packet corrupted or not the expected seq
